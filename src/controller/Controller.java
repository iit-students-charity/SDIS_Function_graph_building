...
public class Controller {private Function arrayFunction;
    ...
    public void startGraphicBuilding(int numberOfLists) {
        if (!isTCalcThreadsAlive) {
            ...
            arrayFunCalcThread = new Thread(new SortingTask(
                    arrayFunction, numberOfLists, functionOrders.get(arrayFunction), graphicCanvas
            ));
            linFunCalcThread = new Thread(new LinearFunctionCalcTask(
                    linearFunction, functionOrders.get(linearFunction), graphicCanvas
            ));
